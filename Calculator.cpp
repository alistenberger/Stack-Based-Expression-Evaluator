...
bool Calculator::infix_to_postfix (const std::string & infix, Expr_Command_Factory & factory, Array <Expr_Command *> & postfix)
{
  ...
  Stack <Expr_Command *> * tempStack = new Stack <Expr_Command *> ();
  while (!input.eof ()) { 
    input >> token; 
    //Function evaluates a token and returns the corresponding Expression Command utilizing the parameter factory
    command = this->evaluate_token (token, factory);
    //Function handles what to do with the command, if it is a number command, place it in the postfix array, else if it is an operator we use the stack and evaluate its precedence
    this->evaluate_command (command, postfix, tempStack);
  }
...
Expr_Command * Calculator::evaluate_token (std::string token, Expr_Command_Factory & factory)
{
...
void Calculator::evaluate_command (Expr_Command * command, Array <Expr_Command *> & postfix, Stack <Expr_Command *> * tempStack)
{
